...
def get_aligned_NTU(save=False, rand=999999):
    ...
    if save:
        if rand ==999999:
            np.save("/hpc/group/tdunn/action_data/FullDataMetaTemp.npy", meta)
            np.save("/hpc/group/tdunn/action_data/FullDataTemp.npy", NTUfull)
            np.save("/hpc/group/tdunn/action_data/SmoothedDataTemp.npy", avg)
        else:
...
def NTU_to_mat():
    [NTUfull,meta]=get_aligned_NTU(save=True)
    print(np.shape(NTUfull))
    print(np.shape(meta))
    ...
    with open('NTUmetadata60clean.csv', 'w', encoding='UTF8', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["scene","person","take","action"])
        writer.writerows(meta)
    ...
    matout['predictions']=preddict
    print(np.shape(smoothNTU))
    scipy.io.savemat("HumanNTU60clean.mat",matout)
...
print("hey")
df=pd.read_csv("/hpc/group/tdunn/action_data/MSR3D/a01_s01_e01_skeleton.txt")
...
vis.skeleton_vid3D(new,
                        connectivity = conn,
                        title="MSRtry",
                        frames=[int(len(new)/2)],
                        N_FRAMES = len(new)-2,
                        fps=30,
                        VID_NAME="MSR3Dskel.mp4",
                        SAVE_ROOT = "./videos/",
                        fsize=25,
                        aspect=[0.9,0.9,1.5],
                        offset=[0.1,0.1])
